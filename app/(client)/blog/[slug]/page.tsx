                ...
                {blog?.blogcategories?.map(
                  (item: { title: string | null; slug?: string | null }, index: number) => (
                    item?.title ? (
                      <p
                        key={index}
                        className="font-semibold text-[var(--color-shop-dark-blue)] tracking-wider"
                      >
                        {item.title}
                      </p>
                    ) : null // safely skip if title is null
                  )
                )}
                <span className="absolute left-0 -bottom-1.5 bg-[var(--color-shop-light-Color)]/30 inline-block w-full h-[2px] group-hover:bg-[var(--color-shop-dark-blue)] hover:cursor-pointer hoverEffect" />
              </div>
